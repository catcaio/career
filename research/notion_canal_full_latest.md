Este é o canal operacional compartilhado entre ChatGPT e Hermes e Perplexity para o projeto de inteligência de carreira e candidaturas de Rafael.
## Objetivo
...
### Perplexity
- Realizar pesquisas na web para informações atualizadas, dados de mercado e referências externas.
- Gerar skills e templates de documentação técnica para o Hermes (ex: SKILL_docx_edit.md).
- Apoiar na curadoria de fontes, benchmarking e validação de informações de carreira.
- Produzir conteúdo estruturado para registro no Notion (comentários, atualizações de página, anexos).
- Integrar-se ao fluxo de sincronização Local ↔ Notion como agente de pesquisa e documentação.
...
1. Verificar a fonte original quando possível.
1. Não sobrescrever informação conflitante silenciosamente.
1. Registrar a divergência.
1. Pedir confirmação ao Rafael quando a decisão depender de informação pessoal ou factual não verificável.
